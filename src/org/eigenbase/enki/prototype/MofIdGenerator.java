...
public class MofIdGenerator extends TableGenerator
{
    ...
    public void configure(Type type, Properties params, Dialect d)
    {
        ...
        synchronized(MofIdGenerator.class) {
            if (interval <= 0) {
                interval = 
                    PropertiesHelper.getInt(
                        INTERVAL_PARAM, params, Short.MAX_VALUE);
                ...
                if (interval < 2 || interval > max) {
                    throw new MappingException(
                        INTERVAL_PARAM + " parameter must be between 2 and " +
                        max + ", inclusive");
                }
                subInterval = -1;
                high = 0;
    ...
    public synchronized Serializable generate(
        SessionImplementor session, Object obj) throws HibernateException
    {
        synchronized(MofIdGenerator.class)
        {
            ...
            Number result = IdentifierGeneratorFactory.createNumber(
                high + subInterval++,
                resultType );
