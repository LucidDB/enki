...
/**
 * MofImplementationGenerator generates code for an in-memory MOF model that
 * can be used to implement any metamodel.  This generator is typically not
 * used except during development.
 * 
        ...
        StructureHandler
{
        ...
        throws GenerationException
    {
        ...
        try {
            ...
            startBlock(
                "public ",
                generator.getSimpleTypeName(assoc),
                "(", REF_PACKAGE_CLASS, " container)");
            writeln(
                "super(");
            increaseIndent();
            writeln("container,");
            writeln(QUOTE, assocInfo.names[0], QUOTE, ",");
            writeln(MULTIPLICITY_CLASS, ".", end1Multiplicity, ",");
            writeln(QUOTE, assocInfo.names[1], QUOTE, ",");
            ...
            if (assocInfo.isChangeable(0) && assocInfo.isChangeable(1)) {
                ...
                writeln(
                    ...
                    assocInfo.names[0], ", ", assocInfo.names[1], ");");
                endBlock();
    ...
    private void generateAssociationEndAccessor(
        ...
        int fromIndex)
    {
        if (assocInfo.ends[getIndex].isNavigable()) {
            ...
            startGenericMethodBlock(
                assoc,
                getEndSingle 
                    ? assocInfo.types[getIndex]
                    : generator.getCollectionType(
                        ordered 
                            ? ORDERED_COLLECTION_CLASS
                            : COLLECTION_CLASS,
                        assocInfo.types[getIndex]),
                generator.getAccessorName(assocInfo.ends[getIndex], null),
                new String[] { assocInfo.types[fromIndex] },
                new String[] { assocInfo.names[fromIndex] });
            if (getEndSingle) {
            ...
            if (getEndSingle) {
                ...
                String cast = assocInfo.types[getIndex];
        ...
        try {            
            ...
            for(Reference ref: instanceReferences) {
                ...
                ReferenceInfo refInfo = 
                    new ReferenceInfo(generator, ref, assoc);

                writePrivateField(
                    refInfo.assocInterfaceName,
                    refInfo.fieldName, 
                    false, 
                    false);
            ...
            if (!instanceReferences.isEmpty()) {
                ...
                for(Reference ref: instanceReferences) {
                    ...
                    writeln(
                        ...
                        QUOTE, refInfo.assoc.getName(), QUOTE,
                        ");");
                }
            }
            endBlock();
            ...
            if (instanceAttributes.size() > 0) {
                ...
                startConstructorBlock(cls, paramTypes, paramNames, "");
        ...
        throws GenerationException
    {
        ...
        try {
            ...
            if (isContained) {
                startBlock(
                    "public ",
                    generator.getSimpleTypeName(pkg, PACKAGE_SUFFIX),
                    "(", REF_PACKAGE_CLASS, " container)");
                writeln("super(container);");
            } else {
                startBlock(
                ...
                nameIter.hasNext() && clsIter.hasNext(); )
            {
                writeln(
                    ...
                    " = new ",
                    generator.getSimpleTypeName(
                        clsIter.next(), CLASS_PROXY_SUFFIX),
                    "(this);");
            }
    ...
    {
        final Association assoc;
        final String assocInterfaceName;
        final String assocTypeName;
        final AssociationKindEnum kind;
        final AssociationEnd referencedEnd;
        
        final Classifier referencedType;
        ...
        final boolean isFirstEnd;
        
        private ReferenceInfo(
            Generator generator, Reference ref, Association assoc)
                throws GenerationException
        {
            this.assoc = assoc;
            this.assocInterfaceName = generator.getTypeName(assoc);
            this.assocTypeName = convertToTypeName(assocInterfaceName);
